...
class Playground extends Component {
  ...
  setLiveSettings = ({ formData }) => {
    let needReset = false;
    if (
      formData.inlineField != this.state.liveSettings.inlineField || //) {
      formData.descriptionAsTooltip !=
        this.state.liveSettings.descriptionAsTooltip
    ) {
      needReset = true;
    }
  ...
  render() {
    ...
    let formContext = {
      onChange: (idSchema, value) => {
        ...
        this.setState({ formData: newFormData, shareURL: null });
      }
    };
    let templateProps = {};
    if (liveSettings.inlineField) {
      if ("antd" === theme) {
        formContext = {
          ...formContext,
          labelCol: {
    ...
    if (FieldTemplate) {
      templateProps.FieldTemplate = FieldTemplate;
    }
    if (extraErrors) {
            ...
            <DemoFrame
              head={
                <React.Fragment>
                  ...
                  />
                  <style dangerouslySetInnerHTML={{ __html: atomCss }} />
